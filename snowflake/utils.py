...
#removed schema: `schemaname`.`tablename` to just `tablename`
#remove limt 1 from UPDATE and DELETE
def generate_sql_pattern(binlog_event, row=None, flashback=False, no_pk=False):
    ...
    if flashback is True:
        ...
        if isinstance(binlog_event, WriteRowsEvent):
            ...
            template = 'DELETE FROM `{0}` WHERE {1};'.format(
                binlog_event.table, ' AND '.join(map(compare_items, row['values'].items())))
            values = map(fix_object, row['values'].values())
        elif isinstance(binlog_event, UpdateRowsEvent):
            template = 'UPDATE `{0}` SET {1} WHERE {2};'.format(
                binlog_event.table,
